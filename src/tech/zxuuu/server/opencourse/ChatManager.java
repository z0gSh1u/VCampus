...
public class ChatManager extends Thread{
	...
	private String toEmoticon(String str) {
		...
		while(curPos > -1) {
			...
			if(!"".equals(emo)) {
				str = str.substring(0, curPos) + emo + str.substring(endPos+1);System.out.println(str);
			}
	...
	public void run() {
		try{
			...
			while(true) {
				userLists.get(-1).add(new ChatSocket(serverSocket.accept(), (String str, ChatSocket chatSocket)->receive(str, chatSocket)));
